...
const deploySchema = z.object({
  gitUrl: z.string().url('gitUrl must be a valid URL (e.g. https://github.com/user/repo)').optional().or(z.literal('')),
  templateId: z.enum(['modern-landing-page', 'react-vite-app', 'analytics-dashboard', '']).optional(),
  projectName: z.string().min(1, 'projectName must be at least 1 character').max(64, 'projectName must not exceed 64 characters').regex(/^[a-zA-Z0-9-_ ]+$/, 'projectName can only contain alphanumeric characters, hyphens, and underscores').optional(),
