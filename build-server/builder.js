...
class BuildWorker {
  ...
  /**
   * Detect modern languages and frameworks in workspace (delegates to modular detector)
   */
  detectLanguageAndFramework(buildFolder) {
    return detectLanguageAndFramework(buildFolder);
  }
  ...
  async executeBuild(deploymentPayload) {
    ...
    try {
      ...
      const deployedUrl = resolveDeployedUrl(deploymentPayload.baseUrl, projectSlug);
  ...
  async updateDeploymentStatus(deploymentId, projectSlug, status, metadata = {}) {
    return updateDeploymentStatus(redis, deploymentId, projectSlug, status, metadata);
  }
  ...
  async executeDirectDeploy(payload) {
    ...
    try {
      ...
      const writtenFilesCount = writeDirectFiles(buildFolder, files, html, css, js);
      ...
      const deployedUrl = resolveDeployedUrl(baseUrl, projectSlug);
      const durationMs = Date.now() - startTime;
